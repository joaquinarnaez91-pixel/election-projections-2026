...
def fetch_onpe_data():
    ...
    try:
        response = requests.get(
            f"{BASE_URL}/totales",
            params={'idEleccion': 10, 'tipoFiltro': 'nacional'},
            timeout=10
        )
...
def calculate_projections(candidates_df):
    ...
    for _, row in candidates_df.iterrows():
        ...
        current_national_completion = 0.65
...
def create_dashboard():
    ...
    if not data:
        ...
        st.info("La API puede estar temporalmente no disponible.")
        return
    ...
    for item in data:
        if isinstance(item, dict):
            candidates.append({
                'candidate_name': item.get('nombreAgrupacionPolitica', 'Unknown'),
                'current_votes': int(item.get('totalVotosValidos', 0)),
                'percentage': float(item.get('porcentajeVotosValidos', 0))
            })
    ...
    with st.expander("📖 Metodología"):
        ...
        **Supuestos:**
        ...
        - Provincias: 45% contabilizado
        - Lima = 32% del electorado
        - Provincias = 68% del electorado
        """)
